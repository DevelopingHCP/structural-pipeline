...
if [ -f $T1 ] ; then
    ...
    if [ ! -f $outwb/$subj.ribbon.nii.gz ];then 
        ...
        run mirtk padding $outtmp/tissue_labels_L.nii.gz $outtmp/tissue_labels_L.nii.gz $outtmp/in_L.nii.gz 2 $CSF_TISSUE_LABEL $GM_TISSUE_LABEL 0
        run fslmaths $outtmp/in_L.nii.gz -bin -mul $LeftGreyRibbonValueIn $outtmp/in_L.nii.gz
        run fslmaths $outtmp/tissue_labels_L.nii.gz -thr $GM_TISSUE_LABEL -uthr $GM_TISSUE_LABEL -bin -mul $LeftGreyRibbonValue $outtmp/out_L.nii.gz

        run mirtk padding $outtmp/tissue_labels_R.nii.gz $outtmp/tissue_labels_R.nii.gz $outtmp/in_R.nii.gz 2 $CSF_TISSUE_LABEL $GM_TISSUE_LABEL 0
        run fslmaths $outtmp/in_R.nii.gz -bin -mul $RightGreyRibbonValueIn $outtmp/in_R.nii.gz
        run fslmaths $outtmp/tissue_labels_R.nii.gz -thr $GM_TISSUE_LABEL -uthr $GM_TISSUE_LABEL -bin -mul $RightGreyRibbonValue $outtmp/out_R.nii.gz
